...
function buildHtml(project: any, roi: any, cost: any) {
  const decisionSummary = roi?.assumptions?.decisionSummary
  ...
  const forecastRows = Array.isArray(project?.forecasts)
    ? project.forecasts
        .map(
          (forecast: any) =>
            `<tr><td>${forecast.channelOrCustomer}</td><td>${forecast.priceBasisConfirmed ? 'Yes' : 'Needs review'}</td><td>$${forecast.monthlyMarketingSpend ?? 0}</td><td>$${forecast.marketingCostPerUnit ?? 0}</td><td>$${forecast.customerAcquisitionCostPerUnit ?? 0}</td></tr>`
        )
        .join('')
  : ''
  const forecastTable = forecastRows
    ? `<h3>Forecast Channel Economics</h3><table border="1" cellpadding="4" cellspacing="0"><tr><th>Channel</th><th>Net Price Confirmed</th><th>Monthly Spend</th><th>Marketing / Unit</th><th>CAC / Unit</th></tr>${forecastRows}</table>`
    : ''
  ...
  return `
    ...
    <ul>
      <li>Verdict: ${decisionSummary.verdict}</li>
      <li>Downside survives: ${decisionSummary.downside?.survives ? 'Yes' : 'No'}</li>
      <li>Downside NPV: $${decisionSummary.downside?.npv ?? 0}</li>
      <li>Downside IRR: ${(((decisionSummary.downside?.irr ?? 0) as number) * 100).toFixed(1)}%</li>
    </ul>
    <p><strong>Next action:</strong> ${decisionSummary.nextAction}</p>
    <p><strong>Missing review items:</strong> ${(decisionSummary.missingReviewItems ?? []).length ? decisionSummary.missingReviewItems.join('; ') : 'None'}</p>
    `
        : ''
    }
    ${forecastTable}
    ...
    <p><strong>Launch Cash Requirement:</strong> ${cost?.launchCashRequirement == null ? 'Needs review' : `$${cost.launchCashRequirement}`}</p>
    <p><strong>Compliance Cost:</strong> ${cost?.complianceCost == null ? 'Needs review' : `$${cost.complianceCost}`}</p>
    <p><strong>Fulfillment / Unit:</strong> ${cost?.fulfillmentCostPerUnit == null ? 'Needs review' : `$${cost.fulfillmentCostPerUnit}`}</p>
    <p><strong>Warranty Reserve:</strong> ${cost?.warrantyReservePct == null ? 'Needs review' : `${(cost.warrantyReservePct * 100).toFixed(1)}%`}</p>
